...
export default function RegisFormPage() {
  ...
  const [plan, setPlan] = useState(""); //ยังไม่เลือกอะไรให้เป็น string ว่างเปล่า
  ...
  const selectPlanOnChange = (event) => {
    //รันเมื่อ select เปลี่ยนค่า
    setPlanError(false);
    setPlan(event.target.value);
  };

  const radioGenderMaleOnChange = () => {
    //ถูกclickป่าว ไม่่ได้เอาไปใช้อะไรต่อ
  ...
  const cbAgreeOnChange = (event) => {
    //check box
    setAgree(event.target.checked); //ติ้กถูกส่งtrue
  };
      ...
      <div>
        ...
        <div>
          <input
            ...
            checked={gender === "male"} //ถูกติ้กเมื่อ gender = male
          />
          Male 👨
          <input
            ...
            checked={gender === "female"} //ถูกติ้กเมื่อ gender = female
          />
          Female 👩
